...
(function () {
  ...
  const APP_VERSION = 'v29';   // shown in the corner so you can confirm the live build (bump with the SW version)
  ...
  function renderPanels() {
    state.players.forEach((p, i) => {
      ...
      const cards = RES.reduce((n, r) => n + p.resources[r], 0) - ((ui.cardLag && ui.cardLag[p.color]) || 0);   // hold count behind in-flight cards
  ...
  function showDiceReveal(dice) {
    ...
    setTimeout(() => {
      ...
      const pm = productionMap(); ui.cardLag = {};
  ...
  // each landed card ticks the owner's displayed count up by one; clearCardLag snaps it to the true total
  function bumpCardLag(color) { if (ui.cardLag && ui.cardLag[color] > 0) { ui.cardLag[color]--; renderPanels(); } }
  function clearCardLag() { if (ui.cardLag && Object.keys(ui.cardLag).length) { ui.cardLag = {}; renderPanels(); } }

...
(function () {
  ...
  const APP_VERSION = 'v64';   // shown in the corner so you can confirm the live build (bump with the SW version)
  ...
  function statsScreen() {
    ...
    const body = games.length
      ? `<div class="sttbl-wrap"><table class="sttbl"><thead>${head}</thead><tbody>${rows}</tbody></table></div>
         ...
         <h4 class="stsub">Recent games${selLbl ? ' · ' + selLbl : ''}${sizeLbl}</h4>
    ...
    titleCard(`${back}
      ...
      ${countSeg}
      ${body}`);
    // pills go oldest→newest, so the active (newest) one sits toward the right — scroll each strip to it
    requestAnimationFrame(() => document.querySelectorAll('.stseasonseg').forEach((seg) => {
      const on = seg.querySelector('button.on');
      if (on) seg.scrollLeft += on.getBoundingClientRect().right - seg.getBoundingClientRect().right + 12;
    }));
  }

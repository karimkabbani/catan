...
(function () {
  ...
  const APP_VERSION = 'v126';   // shown in the corner so you can confirm the live build (bump with the SW version)
  ...
  function showLobby() {
    ...
    LOBBY.join();
    // fresh login lands AT the standing table — the group's meeting point — once per session.
    // The ← from there shows the full games list (and won't bounce back here).
    if (!LOBBY.table && !LOBBY.autoHomed) { LOBBY.autoHomed = true; LOBBY.enterTable(HOME_CODE); }

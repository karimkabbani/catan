...
(function () {
  ...
  const APP_VERSION = 'v111';   // shown in the corner so you can confirm the live build (bump with the SW version)
  ...
  const LOBBY = {
    ...
    gameName(code) { if (code === HOME_CODE) return 'The Table'; const c = this.creatorOf(code); return c ? c.name + "'s Game" : 'Game'; },
  ...
  function tableListHTML(all) {
    ...
    const rowFor = (code, m) => {
      ...
      const gname = home ? '🏠 The Table' : creator ? escapeHtml(creator.name) + "'s Game" : 'Game';

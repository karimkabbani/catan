...
(function () {
  ...
  const APP_VERSION = 'v128';   // shown in the corner so you can confirm the live build (bump with the SW version)
  ...
  function saveSettings() {
    try { localStorage.setItem('catan-settings', JSON.stringify(SETTINGS)); } catch (_) {}   // instant local cache
    // settings follow the LOGIN (like quick messages) — push to the profile, debounced
    if (AUTH && AUTH.me) {
      clearTimeout(settingsPush);
      settingsPush = setTimeout(() => { AUTH.setSettings({ music: SETTINGS.music, sfx: SETTINGS.sfx, anim: SETTINGS.anim, autozoom: SETTINGS.autozoom }); }, 600);
    }
  }
  // profile settings arriving at login/resume override the device cache
  function applyProfileSettings(s) {
    if (!s || typeof s !== 'object') return;
    ['music', 'sfx', 'anim', 'autozoom'].forEach((k) => { if (s[k] !== undefined) SETTINGS[k] = s[k]; });
    try { localStorage.setItem('catan-settings', JSON.stringify(SETTINGS)); } catch (_) {}
  }
  ...
  // iOS quirk: the RINGER/SILENT switch mutes Web Audio (our sound effects) but NOT <audio>
  // elements (our music). Holding the session open with a silent looping <audio> promotes the
  // page to "playback" audio, so SFX play through the mute switch — same as the music does.
  let sessionEl = null;
  function silentWavURI() {
    const rate = 8000, n = rate / 2;   // 0.5s of silence
  ...
  function keepAudioSession() {
    if (!SETTINGS.sfx) { if (sessionEl && !sessionEl.paused) { try { sessionEl.pause(); } catch (_) { } } return; }
    if (!sessionEl) { sessionEl = new Audio(silentWavURI()); sessionEl.loop = true; sessionEl.volume = 0.01; }
    if (sessionEl.paused) sessionEl.play().catch(() => { });   // lands inside the tap gesture; retried on the next tap
  }
  function unlockAudio() { if (!actx) initAudio(); if (actx && actx.state === 'suspended') actx.resume(); keepAudioSession(); startMusic(); }
  ...
  function afterAction() {
    ...
    // only auto-close the "X is discarding…" status itself (#dwait) — a compose box that was
    // left open through the discards must not be swept away with it
    if (ui.discardWait && state.turnPhase !== 'discard') { ui.discardWait = false; if ($('dwait')) hideOverlay(); }
  ...
  function promptDiscards() {
    ...
    if (!online || cur.color === myColor) {
      // the discard is mandatory and takes the screen — but never orphan an active voice
      // recording (stop it cleanly) and the typed draft survives via bcDraft.
      if (VOICE.recording) finishRec(true);
      ...
      ui.discardWait = true;
      // ...but don't steal an active compose box or voice recording for a purely
      // informational status — the banner already says whose turn it is.
      if ($('bcInput') || document.querySelector('#overlay .vrec')) return;
      showOverlay(`<h3>Discarding</h3><p id="dwait" class="muted" style="text-align:center;margin:10px 0"><b style="color:var(--gold)">${escapeHtml(cur.name)}</b> is discarding…</p>`);
    }
  }
  ...
  function showVictory() {
    ...
    showOverlay(`<div class="winwrap${domination ? ' domination' : ''}">
      ...
      ${online
        ? (myColor
          ? `<button class="btn full" style="margin-bottom:9px" onclick="CATAN.rematch()">🔁 Rematch — same players</button><button class="btn wood full" onclick="CATAN.tableReset()">Back to lobby</button>`
          : `<button class="btn full" onclick="CATAN.confirmExit()">Back to lobby</button>`)   /* a watcher leaves for themselves — never resets the players' table */
  ...
  // draft survives forced interruptions (a 7 forcing YOU to discard replaces the overlay) —
  // kept on every keystroke, restored next time the box opens, cleared on send.
  let bcDraft = '';
  function openBroadcast() {
    if (!online || !myColor) return;
    showOverlay(`<h3>Quick message</h3>
      <input id="bcInput" class="authin" maxlength="50" placeholder="Say something…" autocomplete="off" value="${escapeHtml(bcDraft)}" oninput="CATAN.bcKeep(this.value)">
  ...
  function sendBroadcastMsg() {
    ...
    bcDraft = '';   // sent (or deliberately submitted empty) -> the draft is done
    hideOverlay(); render();
    sendBroadcastText(text);
  }
  ...
  window.CATAN = {
    ...
    openSettings,
    setToggle: (key) => { SETTINGS[key] = !SETTINGS[key]; saveSettings(); openSettings(); if (key === 'music') { if (SETTINGS.music) startMusic(); else stopMusic(); } if (key === 'sfx') keepAudioSession(); },
  ...
  // Seat-color assignment honouring player preferences (profile -> presence `pref`).
  // Each contested color goes to ONE random claimant; everyone else draws from the
  // remaining colors in seat order. Runs only on the host (single writer), so plain
  // Math.random is safe — the result ships to everyone in the game row.
  function assignSeatColors(seated) {
    const byId = {}, taken = new Set();
    const wants = {};   // color -> [ids that asked for it]
    seated.forEach((p) => { if (p.pref && SEAT_COLORS.includes(p.pref)) (wants[p.pref] = wants[p.pref] || []).push(p.id); });
    for (const c of SEAT_COLORS) {
      const w = wants[c]; if (!w || !w.length) continue;
      byId[w[(Math.random() * w.length) | 0]] = c;   // random winner among the claimants
      taken.add(c);
    }
    const rest = SEAT_COLORS.filter((c) => !taken.has(c));
    seated.forEach((p) => { if (!byId[p.id]) byId[p.id] = rest.shift(); });
    return byId;
  }
  ...
  function cinematicPlace(kind, id, mine, grant) {
    ...
    if (setup) {
      ...
      // The 2nd-settlement starting cards MUST fly no matter what the camera does — dispatch
      // already held the counts (lagIn), and only the landing cards tick them back up. landCard
      // is a no-op once the lag was cleared, so flying after a cancel can't double-count.
      const flyGrant = () => { if (mine && grant && grant.items && grant.items.length) flySettlementGrant(grant); };
      // auto-zoom off (runCine would bail): no camera hold — just fly the cards at the current view
      if (!SETTINGS.autozoom || !zArea) { flyGrant(); return; }
      ...
      runCine(async (alive) => {
        ...
        await cineSleep(aDur(CINE_MS + 80));
        flyGrant();   // even if superseded mid-glide — the cards still land and settle the counts
        if (!alive()) return;
        if (mine && grant && grant.items && grant.items.length) await cineSleep(aDur(grant.items.length * 430 + 450));   // linger while they stream in
      }, { stay: true });
      return;
    }
  ...
  // The group's standing table: always exists, pinned at the top of the lobby, and where a fresh
  // login lands. No creator — the first player to ready up hosts (startTable/atTableHTML fall back
  // to ready[0]). 5 chars so a random 4-char genCode can never collide with it.
  const HOME_CODE = 'TABLE';
  ...
  const NET = {
    ...
    subscribe() {
      ...
      this.poll = setInterval(async () => {
        if (!NET.code) return;
        const { data, error } = await c.from('games').select('*').eq('code', NET.code).maybeSingle();
        if (error) return;                      // transient network/DB blip -> keep the state we have (never boot on a failed fetch)
    ...
    onRow(row) {
      ...
      if (this.started && row.version < this.version) {
        // A lower version is EITHER a stale read (a poll response that raced a realtime update —
        // common) or the row was genuinely replaced (table reset + new game while this device
        // slept). Distinguish by GAME IDENTITY: the same board means it's still my game.
        const bfp = (s) => (((s && s.board && s.board.hexes) || []).map((h) => ((h.terrain || '?')[0]) + (h.token || '')).join(''));
        if (state && row.state && bfp(state) === bfp(row.state)) return;   // stale read of my own game -> ignore
        this.started = false; online = false; myColor = null;
        const seat2 = (row.players || []).find((p) => p.playerId === (AUTH.me && AUTH.me.id));
        if (!seat2 && LOBBY.mode !== 'spectate') { this.version = row.version; LOBBY.lastRow = row; gameSeats = row.players || null; toast('A new game started at this table'); LOBBY.toLobby(); LOBBY.inProgress = true; lobbySig = null; renderLobby(); return; }
        // seated (or spectating) in the new game -> fall through and enter it like a fresh join
      }
  ...
  const LOBBY = {
    ...
    join() {
      ...
      // Presence is DB-BACKED (heartbeat row + poll, server-stamped ages): the realtime socket dies
      // silently on mobile, so "who's online" must never depend on it — the same lesson that moved
      // chat and game state onto polled tables. Different phones can no longer see different rosters.
    ...
    myPayload() { return { id: AUTH.me.id, name: AUTH.me.name, avatar: AUTH.me.avatar || null, pref: this.myPref(), mode: this.mode, readyAt: this.readyAt, table: this.table || null, created: this.created || null, target: this.targetPoints || null }; },
    // announce my state: heartbeat row on the server + an optimistic local echo so my own UI updates
    // instantly (the poll confirms it a few seconds later)
    track() {
      if (!AUTH.me) return;
      const p = this.myPayload();
      this.presence[p.id] = Object.assign({}, this.presence[p.id] || {}, p, { away: false }); this.liveIds.add(p.id);
      const c = NET.init(); if (c) c.rpc('presence_beat', { p }).then(() => { }, () => { });
    },
    creatorOf(code) { return this.online().find((p) => (p.table || null) === code && p.created === code) || null; },
    gameName(code) { if (code === HOME_CODE) return 'Main Game'; const c = this.creatorOf(code); return c ? c.name + "'s Game" : 'Game'; },
    ...
    // fetch the roster from the DB. A failed fetch keeps the roster we have (a blip never empties the lobby).
    async pollPresence() {
      const c = NET.init(); if (!c || !AUTH.me) return;
      const { data, error } = await c.rpc('presence_list');
    ...
    applyPresence(rows) {
      ...
      rows.forEach((r) => {
        if (!r || !r.id || r.mode === 'gone') return;   // 'gone' = logged out on purpose
        const m = { id: r.id, name: r.name, avatar: r.avatar, pref: r.pref, mode: r.mode, readyAt: Number(r.ready_at) || 0, table: r.table_code || null, created: r.created || null, target: r.target || null };
        const age = Number(r.age_s) || 0;
        if (age < 18) { this.liveIds.add(r.id); live[r.id] = m; }               // fresh heartbeat (5s cadence + slack)
        else live[r.id] = Object.assign(m, { away: true });                     // phone asleep — keep in the roster, dimmed
      });
      // my own entry reflects LOCAL truth — a beat may still be in flight after a mode change
      if (AUTH.me) { const p = this.myPayload(); live[p.id] = Object.assign(live[p.id] || {}, p, { away: false }); this.liveIds.add(p.id); }
      this.presence = live;
    ...
    enterAs(role) { this.mode = role === 'play' ? 'playing' : 'spectate'; this.readyAt = 0; this.inProgress = false; this.track(); },
    toLobby() { this.mode = 'idle'; this.readyAt = 0; this.inProgress = false; this.track(); showLobby(); },   // table idle -> back to lobby
    ...
    async startTable() {
      ...
      const c = NET.init();
      // never start OVER a live game — a stale ready-screen (slow fetch, resumed PWA) must not
      // clobber a match in progress on this code. Check the row right before writing.
      const { data: cur } = await c.from('games').select('*').eq('code', NET.code).maybeSingle();
      if (cur && cur.phase === 'playing' && cur.state) { toast('A game is already running at this table'); NET.onRow(cur); return; }
    ...
    async rematch() {
      ...
      const players = seats.map((p, i) => ({ seat: i, color: p.color, name: p.name, playerId: p.playerId || null, avatar: p.avatar || null }));   // rematch keeps everyone's color
    ...
    leave() {
      const c = NET.init();
      if (c && AUTH.me) { try { c.rpc('presence_beat', { p: Object.assign(this.myPayload(), { mode: 'gone', table: null }) }).then(() => { }, () => { }); } catch (_) { } }   // drop off the roster immediately
      if (c && this.channel) { try { c.removeChannel(this.channel); } catch (_) { } this.channel = null; }
  ...
  function onWake() {
    ...
    try { LOBBY.track(); LOBBY.pollPresence(); } catch (_) { }
  ...
  function tableListHTML(all) {
    ...
    const codes = Object.keys(tables).filter((c) => c !== HOME_CODE).sort();   // the standing table renders pinned, above
    const browsing = all.filter((p) => !p.table).length;
    const rowFor = (code, m) => {
      const home = code === HOME_CODE, inGame = m.some((p) => pmode(p) === 'playing');
      const creator = m.find((p) => p.created === code);
      const gname = home ? '🏠 Main Game' : creator ? escapeHtml(creator.name) + "'s Game" : 'Game';
      const names = m.map((p) => escapeHtml(p.name)).join(', ');
      return `<button class="tablerow${home ? ' home' : ''}" onclick="CATAN.joinTable('${code}')"><span class="trow-main"><span class="tgname">${gname}</span><span class="tgwho">${names || (home ? 'Empty — gather here' : 'Empty')}</span></span>` +
        `<span class="${inGame ? 't-pend' : 't-acc'}">${inGame ? '🔴 watch' : m.length ? m.length + ' here' : 'join'}</span></button>`;
    };
    const rows = rowFor(HOME_CODE, tables[HOME_CODE] || []) + codes.map((code) => rowFor(code, tables[code])).join('');
  ...
  function atTableHTML(all) {
    ...
    const chosen = iAmHost ? LOBBY.targetPoints : (host ? host.target : null);   // host may be first-ready (standing table has no creator)
  ...
  window.CATAN.newTable = () => {
    // creation guard: if friends are already gathered somewhere waiting to play, joining them is
    // almost always what you meant — creating a second table splits the group.
    const tables = {};
    LOBBY.online().forEach((p) => { if (p.table) (tables[p.table] = tables[p.table] || []).push(p); });
    const waiting = Object.keys(tables)
      .filter((code) => !tables[code].some((p) => pmode(p) === 'playing'))   // a table already mid-game can't be joined anyway
      .sort((a, b) => tables[b].length - tables[a].length)[0];
  ...
  // both guard exits rebuild the lobby shell — the guard card replaced it (titleCard swaps the whole card)
  window.CATAN.guardJoin = (code) => { lobbyShell(); LOBBY.enterTable(code); };
  window.CATAN.newTableGo = () => { const code = genCode(); LOBBY.created = code; lobbyShell(); LOBBY.enterTable(code); };
  ...
  const AUTH = {
    ...
    async resume() {
      ...
      if (r && r.ok) { this.me = { id: r.id, name: r.name, token: s.token, avatar: r.avatar || null, quickMsgs: Array.isArray(r.quick_msgs) ? r.quick_msgs : null, prefColor: r.pref_color || null }; applyProfileSettings(r.settings); return true; }
      return false;
    },
    async login(name, pin) {
      const r = await this.rpc('player_login', { p_name: name, p_pin: pin });
      if (r && r.ok) { this.save({ id: r.id, name: r.name, token: r.token, avatar: r.avatar || null, quickMsgs: Array.isArray(r.quick_msgs) ? r.quick_msgs : null, prefColor: r.pref_color || null }); applyProfileSettings(r.settings); return { ok: true }; }
    ...
    async setSettings(s) { if (!this.me) return { ok: false, error: 'Not logged in' }; return await this.rpc('player_set_settings', { p_token: this.me.token, p_settings: s }); },
    // preferred seat color ('red'|'blue'|'green'|'yellow') or null = no preference
    async setPrefColor(color) {
      if (!this.me) return { ok: false, error: 'Not logged in' };
      const r = await this.rpc('player_set_pref_color', { p_token: this.me.token, p_color: color || null });
      if (r && r.ok) { this.me.prefColor = color || null; this.save(this.me); }
      return r;
    },
  ...
  function showLobby() {
    ...
    LOBBY.join();
    renderLobby();     // fill the dynamic half (lands at the games list; Main Game is the pinned row)
  ...
  window.CATAN.setPrefColor = async (c) => {
    const r = await AUTH.setPrefColor(c);
    if (r && r.ok) { LOBBY.track(); manageProfile(); }   // re-track so the lobby/table sees the new preference
    else toast((r && r.error) || 'Could not save color');
  };

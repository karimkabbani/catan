...
describe('forceSkip escape hatch', () => {
  it('skips the current player and advances to the next, invocable by anyone', () => {
    let s = runSetup(createGame({ id: 'fs1', players: PLAYERS, seed: 3 }));
    expect(currentColor(s)).toBe('red');
    // roll first so red is mid-turn, then skip red — invoked by a DIFFERENT player
    const rr = applyAction(s, { type: 'rollDice' }, 'red');
    expect(rr.ok).toBe(true);
    if (!rr.ok) return;
    s = rr.state;
    const before = currentColor(s);
    const r = applyAction(s, { type: 'forceSkip' }, 'blue');   // not red's move, still allowed
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    s = r.state;
    expect(currentColor(s)).not.toBe(before);
    expect(s.turnPhase).toBe('roll');
    expect(s.hasRolledThisTurn).toBe(false);
    expect(conserved(s)).toBe(true);
  });

  it('skips a player stuck before rolling (no dice yet)', () => {
    let s = runSetup(createGame({ id: 'fs2', players: PLAYERS, seed: 8 }));
    expect(s.turnPhase).toBe('roll');
    const r = applyAction(s, { type: 'forceSkip' }, 'blue');
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    s = r.state;
    expect(currentColor(s)).toBe('blue');
    expect(s.turnPhase).toBe('roll');
  });

  it('auto-discards for a named gone player and unsticks the robber', () => {
    let s = runSetup(createGame({ id: 'fs3', players: PLAYERS, seed: 5 }));
    // give red a fat hand (moved from the bank, so conservation still holds) -> owes a discard
    const red = s.players.find((p) => p.color === 'red')!;
    RESOURCES.forEach((r) => { const take = Math.min(2, s.bank[r]); red.resources[r] += take; s.bank[r] -= take; });
    // manufacture the discard phase directly
    s.turnPhase = 'discard';
    s.pendingDiscards = { red: 5, blue: 0, white: 0 } as any;
    const owedBefore = RESOURCES.reduce((n, r) => n + red.resources[r], 0);
    const r = applyAction(s, { type: 'forceSkip', victims: ['red'] }, 'blue');
    expect(r.ok).toBe(true);
    if (!r.ok) return;
    s = r.state;
    const owedAfter = RESOURCES.reduce((n, r2) => n + s.players.find((p) => p.color === 'red')!.resources[r2], 0);
    expect(owedBefore - owedAfter).toBe(5);
    expect(s.turnPhase).toBe('moveRobber');
    expect(conserved(s)).toBe(true);
  });
});

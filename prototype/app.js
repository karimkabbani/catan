...
(function () {
  ...
  const APP_VERSION = 'v36';   // shown in the corner so you can confirm the live build (bump with the SW version)
  ...
  const STATS = {
    games: [], loaded: false, loading: null,
      ...
      STATS.loading = c.from('game_results').select('standings,finished_at').order('finished_at', { ascending: false }).limit(1000).then(
        ({ data, error }) => {
          ...
          STATS.games = ((!error && data) ? data : []).filter((r) => Array.isArray(r.standings) && r.standings.length).map((r) => {
            ...
            return { date: MONTH_ABBR[mo] + ' ' + d.getDate(), ym: d.getFullYear() + '-' + String(mo + 1).padStart(2, '0'),
              standings: r.standings.map((s) => ({ n: s.name, pts: s.pts, lr: s.lr ? 1 : 0, la: s.la ? 1 : 0 })) };
          });
        },
    ...
    crowns() { const c = {}; STATS.seasons().forEach((ym) => { if (ym === STATS.curSeason()) return; const b = STATS.board(STATS.filter(ym)); if (b.length) c[b[0].name] = (c[b[0].name] || 0) + 1; }); return c; },
    streak(name) {
  ...
  function recordResult() {
    ...
    const standings = rows.map(({ p, vp }, i) => ({ name: p.name, color: p.color, pts: vp, place: i + 1, lr: p.hasLongestRoad ? 1 : 0, la: p.hasLargestArmy ? 1 : 0 }));

...
(function () {
  ...
  function dispatch(action, byColor) {
    ...
    if (action.type === 'buildSettlement' || action.type === 'buildCity') {
      ...
      if (state.phase === 'setup') {
        ...
        if (RES.some((x) => (af[x] || 0) > (b[x] || 0))) {
          ...
          lagIn(actor, items.length);   // hold the count; the grant cards tick it up as they land
        }
      }
    ...
    if (steal) { lagOut(steal.victim, 1); lagIn(steal.thief, 1); }
    if (trade) { lagOut(trade.a, trade.g); lagIn(trade.a, trade.w); lagOut(trade.b, trade.w); lagIn(trade.b, trade.g); }
    afterAction(); render();
  ...
  function renderPanels() {
    state.players.forEach((p, i) => {
      ...
      const cards = RES.reduce((n, r) => n + p.resources[r], 0) - ((ui.cardLag && ui.cardLag[p.color]) || 0) + ((ui.cardOut && ui.cardOut[p.color]) || 0);   // count walks with the card animation
  ...
  function showDiceReveal(dice) {
    ...
    setTimeout(() => {
      ...
      const pm = productionMap(); ui.cardLag = {}; ui.cardOut = {};
      for (const e of pm) ui.cardLag[e.color] = (ui.cardLag[e.color] || 0) + e.count;
      renderPanels(); cinematicRoll(); afterAction(); render();
    }, aDur(1300));
  }
  ...
  function flyResource(res, sx, sy, tx, ty, delay, onland) {
    flyImage((HUD.res && HUD.res[res]) || (ASSETS.icons && ASSETS.icons[res]), sx, sy, tx, ty, delay, { sound: 'res_' + res, vol: 0.45, onland });
  }
  // Displayed count = true total − cards still flying IN + cards still flying OUT, so the number walks
  // with the animation (arriving card ticks the receiver up, departing card ticks the giver down).
  // Set the lag at detection (before the render) so the count never flashes the new value first.
  function lagIn(color, n) { if (n > 0) { ui.cardLag = ui.cardLag || {}; ui.cardLag[color] = (ui.cardLag[color] || 0) + n; } }
  function lagOut(color, n) { if (n > 0) { ui.cardOut = ui.cardOut || {}; ui.cardOut[color] = (ui.cardOut[color] || 0) + n; } }
  function bumpCardLag(color) { if (ui.cardLag && ui.cardLag[color] > 0) { ui.cardLag[color]--; renderPanels(); } }
  function bumpCardOut(color) { if (ui.cardOut && ui.cardOut[color] > 0) { ui.cardOut[color]--; renderPanels(); } }
  function clearCardLag() { let ch = false; if (ui.cardLag && Object.keys(ui.cardLag).length) { ui.cardLag = {}; ch = true; } if (ui.cardOut && Object.keys(ui.cardOut).length) { ui.cardOut = {}; ch = true; } if (ch) renderPanels(); }
    ...
    for (let pi = 0; pi < state.players.length; pi++) {
      ...
      for (const e of map) {
        ...
        for (let k = 0; k < e.count; k++) {
          ...
          flyResource(e.resource, s.x, s.y, tx, ty, delay, () => bumpCardLag(color));
          delay += STAGGER;
        }
      }
    }
  ...
  function showStealFly(victimColor, thiefColor, res) {
    ...
    flyImage(img, vr.left + vr.width / 2, vr.top + vr.height / 2,
      tr.left + tr.width / 2, tr.top + tr.height / 2, 0, { card: true, sound: 'whoosh', vol: 0.55, onlift: () => bumpCardOut(victimColor), onland: () => bumpCardLag(thiefColor) });
  }
  ...
  function showTradeFly(aColor, bColor, nGive, nWant) {
    ...
    playSound('trade', 0.5);
    let d = 0;
    for (let i = 0; i < (nGive || 0); i++) { flyImage('assets/hud/cardback.png', ax, ay, bx, by, d, { card: true, onlift: () => bumpCardOut(aColor), onland: () => bumpCardLag(bColor) }); d += STAG; }
    d += 340;   // brief pause, THEN the return cards come back (sequential, not simultaneous)
    for (let i = 0; i < (nWant || 0); i++) { flyImage('assets/hud/cardback.png', bx, by, ax, ay, d, { card: true, onlift: () => bumpCardOut(bColor), onland: () => bumpCardLag(aColor) }); d += STAG; }
  }
  ...
  function showBankFly(color, giveObj, wantObj, covered) {
    ...
    playSound('trade', 0.5);
    let d = 0;
    RES.forEach((rr) => { for (let i = 0; i < (giveObj[rr] || 0); i++) { flyImage(img(rr), px, py, cx, cy, d, Object.assign({}, opt, { onlift: () => bumpCardOut(color) })); d += STAG; } });
    d += 340;   // all the given cards reach the bank, then your received cards come back
    RES.forEach((rr) => { for (let i = 0; i < (wantObj[rr] || 0); i++) { flyImage(img(rr), cx, cy, px, py, d, Object.assign({}, opt, { onland: () => bumpCardLag(color) })); d += STAG; } });
  }
  ...
  window.CATAN = {
    ...
    tradeBank: () => {
      ...
      lagOut(color, sumObj(t.give)); lagIn(color, sumObj(t.want)); renderPanels();
  ...
  function cancelCine() { if (cineRunning) { cineToken++; cineRunning = false; clearCardLag(); } }   // a manual gesture takes over; snap counts to true totals
  ...
  function flyHexProduction(group, base) {
    ...
    for (const e of group.entries) {
      ...
      for (let k = 0; k < e.count; k++) { flyResource(e.resource, sx, sy, tx, ty, delay, () => bumpCardLag(e.color)); delay += 230; }
    }
  }
  ...
  function flySettlementGrant(grant) {
    ...
    for (const it of grant.items) {
      ...
      const sc = pt.matrixTransform(ctm); flyResource(it.resource, sc.x, sc.y, tx, ty, delay, () => bumpCardLag(grant.color)); delay += aDur(430);   // sequence, one terrain at a time
    }
  }
  ...
  function applyRemoteState(s) {
    ...
    if (stolen) { lagOut(stolen.victim, 1); lagIn(stolen.thief, 1); }
    if (trade) { lagOut(trade.a, trade.g); lagIn(trade.a, trade.w); lagOut(trade.b, trade.w); lagIn(trade.b, trade.g); }
    if (bankTrade) { lagOut(bankTrade.color, sumObj(bankTrade.give)); lagIn(bankTrade.color, sumObj(bankTrade.want)); }
    afterAction(); render();

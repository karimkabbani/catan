...
(function () {
  ...
  const AUTH = {
    ...
    async resume() {
      ...
      if (r && r.ok) { this.me = { id: r.id, name: r.name, token: s.token, avatar: r.avatar || null, quickMsgs: Array.isArray(r.quick_msgs) ? r.quick_msgs : null, prefColor: r.pref_color || null }; return true; }
      return false;
    },
    async login(name, pin) {
      const r = await this.rpc('player_login', { p_name: name, p_pin: pin });
      if (r && r.ok) { this.save({ id: r.id, name: r.name, token: r.token, avatar: r.avatar || null, quickMsgs: Array.isArray(r.quick_msgs) ? r.quick_msgs : null, prefColor: r.pref_color || null }); return { ok: true }; }

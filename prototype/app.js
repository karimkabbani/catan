  ...
  const NET = {
    ...
    onRow(row) {
      ...
      if (this.started && row.version < this.version) {
        // a LOWER version can only mean the row was replaced under us (table reset + a new game
        // on the same standing code while this device slept). Ignoring it would freeze this
        // client forever — drop out of the dead game and rejoin from scratch instead.

...
(function () {
  ...
  let cropImg = null, cropState = null;
  ...
  function onAvatarFile(e) {
    ...
    const img = new Image();
    img.onload = () => { URL.revokeObjectURL(url); openCropper(img); };
    img.onerror = () => { URL.revokeObjectURL(url); toast("Couldn't read that image"); };
    img.src = url;   // <img> decodes HEIC on iOS where createImageBitmap can't
  }
  ...
  window.CATAN.cropCancel = () => { $('cropper').classList.add('hidden'); cropImg = null; };
  window.CATAN.cropConfirm = async () => {
    ...
    $('cropper').classList.add('hidden'); cropImg = null;

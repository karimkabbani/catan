...
(function () {
  ...
  function tableListHTML(all) {
    ...
    }).join('') : `<p class="muted" style="text-align:center;margin:12px 0">No tables yet — start one.</p>`;
    return `<p class="muted small" style="text-align:center">${me} · ${all.length} online${browsing ? ' · ' + browsing + ' browsing' : ''}</p>
      ...
      <button class="btn wood full" onclick="CATAN.newTable()">+ New table</button>`;
  }
  ...
  function atTableHTML(all) {
    ...
    const leave = `<button class="btn ghost full" onclick="CATAN.leaveTable()">← Leave table</button>`;
    if (LOBBY.inProgress) {
      return `<p class="muted small" style="text-align:center">Table · ${members.length} here</p>
        <div class="loblist">${rows}</div>
    ...
    return `<p class="muted small" style="text-align:center">Table · ${members.length} here · ${ready.length} ready</p>
      <div class="loblist">${rows}</div>
      <div class="lobrow2">
